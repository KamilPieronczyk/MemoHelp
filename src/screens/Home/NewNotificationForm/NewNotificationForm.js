...
export function NewNotificationForm() {
	...
	const pushToFirestore = () => {
		...
		for (const [ key, value ] of groupsData.entries()) {
			...
			if (value.selected) {
				reminder
					.SendReminderToGroupCollection(key)
					.then(() => {
	...
	return (
		<FormContainer
			className={
					...
					showMoreSettings ? styles.openedContainer :
					styles.noOpenedContainer
			}
		>
			<Container>
				<div className={styles.firstColumn}>
					...
					<ContentInput
						...
						type="text"
						className={
								textInputState ? styles.successTextInput :
								styles.errorTextInput
						}
						placeholder="O czym Ci przypomnieć?"
						onInput={autoGrowContentInput}
						onChange={(e) => {
					...
					<TimePickersContainer>
						<span
							...
							onClick={() => setDatePickerVersion(true)}
							className={
									datePickerVersion ? styles.opacity100 :
									styles.opacity40
							}
						>
							...
							<MuiPickersUtilsProvider utils={DateFnsUtils}>
								{
									type != Reminder.reminderTypes.special ? <DateTimePicker
										value={selectedDate}
						...
						<span
							style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: 100, marginLeft: 15 }}
							className={
									datePickerVersion ? styles.opacity40 :
									styles.opacity100
							}
							onClick={() => setDatePickerVersion(false)}
						>
				...
				<div
					className={
							showMoreSettings ? styles.noCollapsedSecondColumn :
							styles.collapsedSecondColumn
					}
				>
					<Collapse in={showMoreSettings} timeout={'auto'}>
						Wybierz grupę:
						<GroupsContainer>
							...
							{groupsData.size <= 0 && <span> nie należysz do żadnych grup</span>}
						</GroupsContainer>
					</Collapse>
				</div>
			</Container>
...
const GroupsContainer = styled.div`padding: 15px;`;
